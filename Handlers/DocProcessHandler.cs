    ...
    public class DocProcessHandler
    {
        ...
        private static void ExecuteInternal()
        {
            ...
            try
            {
                ...
                var fullDocText = newDoc.Range().Text;

                _entry.TotalCount = etalonRequestPattern.Matches(fullDocText).Count + etalonResponsePattern.Matches(fullDocText).Count + testPattern.Matches(fullDocText).Count + tagPattern.Matches(fullDocText).Count;
                
                if (_entry.TotalCount == 0)
                {
                    throw new Exception("Нет шаблонов для заполнения. Проверьте исходный файл на существование шаблонов типа #a/1/method или #b/1");
                }

                for (var i = 1; i <= newDoc.Sections.Count; i++)
                {
                    var wordRange = newDoc.Sections[i].Range;

                    var matches = etalonRequestPattern.Matches(wordRange.Text);
                    SetXmlText(matches, tagPattern,  wordRange,  ProcessType.EtalonRequest, ref currentStep);
                   

                    matches = etalonResponsePattern.Matches(wordRange.Text);
                    SetXmlText(matches, tagPattern, wordRange,  ProcessType.EtalonResponse, ref currentStep);
                   

                    matches = testPattern.Matches(wordRange.Text);
                    SetXmlText(matches, tagPattern, wordRange,  ProcessType.Test, ref currentStep);
        ...
        private static void SetXmlText(MatchCollection matches, Regex tagPattern, Word.Range wordRange, ProcessType processType, ref int currentStep)
        {
            if (matches.Count > 0)
            {
        ...
        private static string GetXmlPath(string xmlNumber, ProcessType processType) => processType switch
        {
        ...
        {
            var experssion = (int)decimal.Round((decimal)current / (decimal)_entry.TotalCount * 100);
            return experssion;
        }
    }
}